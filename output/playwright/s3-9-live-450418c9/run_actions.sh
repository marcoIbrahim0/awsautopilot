...
wait_for_one_bundle() {
  local idx pid dir status_file line status detail stage
  while true; do
    for idx in "${!pids[@]}"; do
      pid="${pids[$idx]}"
...
echo "Running ${TOTAL} bundle folder(s) from ${EXECUTION_ROOT} with parallel executions=${PARALLEL_BUNDLE_EXECUTIONS} (max=${MAX_PARALLEL_BUNDLES})"
