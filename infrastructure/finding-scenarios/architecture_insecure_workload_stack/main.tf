...
variable "region" {
  description = "AWS region for the workload architecture."
  type        = string
  default     = "eu-north-1"
}
...
variable "name_prefix" {
  ...
  type        = string
  default     = "security-autopilot-insecure"
}
...
resource "aws_vpc" "workload" {
  ...
  tags                 = merge(local.workload_tags, { Name = "${var.name_prefix}-vpc" })
}
...
resource "aws_subnet" "public_a" {
  ...
  tags                    = merge(local.workload_tags, { Name = "${var.name_prefix}-public-a" })
}

resource "aws_internet_gateway" "workload" {
  vpc_id = aws_vpc.workload.id
  tags   = merge(local.workload_tags, { Name = "${var.name_prefix}-igw" })
}

resource "aws_route_table" "public" {
  vpc_id = aws_vpc.workload.id
  tags   = merge(local.workload_tags, { Name = "${var.name_prefix}-public-rt" })
}
...
resource "aws_security_group" "admin_ports_open" {
  name        = "${var.name_prefix}-admin-open-sg"
  description = "Intentionally vulnerable: admin ports open to the internet."
  vpc_id      = aws_vpc.workload.id
  tags        = merge(local.workload_tags, { Name = "${var.name_prefix}-admin-open-sg" })
}
...
resource "aws_s3_bucket" "public_assets" {
  bucket_prefix = "${var.name_prefix}-public-assets-"
  force_destroy = true
}
...
resource "aws_s3_bucket" "transport_policy_missing_ssl_deny" {
  bucket_prefix = "${var.name_prefix}-ssl-missing-"
  force_destroy = true
}
...
resource "aws_s3_bucket" "sse_s3_only" {
  bucket_prefix = "${var.name_prefix}-sse-s3-"
  force_destroy = true
}
...
resource "aws_s3_bucket" "no_access_logging" {
  bucket_prefix = "${var.name_prefix}-no-access-logs-"
  force_destroy = true
}
...
resource "aws_s3_bucket" "no_lifecycle" {
  bucket_prefix = "${var.name_prefix}-no-lifecycle-"
  force_destroy = true
}
...
EOF
  ...
  tags       = merge(local.workload_tags, { Name = "${var.name_prefix}-ec2" })
}
...
resource "aws_ebs_volume" "workload_data" {
  ...
  type              = "gp3"
  encrypted         = false
  tags              = merge(local.workload_tags, { Name = "${var.name_prefix}-data-volume" })
}
...
resource "aws_ebs_snapshot" "workload_data" {
  volume_id = aws_ebs_volume.workload_data.id
  tags      = merge(local.workload_tags, { Name = "${var.name_prefix}-data-snapshot" })

  depends_on = [terraform_data.disable_snapshot_block_public_access]
}

output "insecure_workload_architecture" {
  value = {
    ec2_instance_id                   = aws_instance.workload.id
    open_admin_security_group_id      = aws_security_group.admin_ports_open.id
    public_bucket                     = aws_s3_bucket.public_assets.id
    no_ssl_deny_bucket                = aws_s3_bucket.transport_policy_missing_ssl_deny.id
    unencrypted_bucket                = aws_s3_bucket.unencrypted_default.id
    sse_s3_bucket                     = aws_s3_bucket.sse_s3_only.id
    no_access_logging_bucket          = aws_s3_bucket.no_access_logging.id
    no_lifecycle_bucket               = aws_s3_bucket.no_lifecycle.id
    ebs_encryption_by_default_enabled = aws_ebs_encryption_by_default.disabled.enabled
    ebs_snapshot_public_access_block  = "disabled_via_cli_expected"
    workload_snapshot_id              = aws_ebs_snapshot.workload_data.id
  }
}
