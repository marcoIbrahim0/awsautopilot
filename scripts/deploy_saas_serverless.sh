...
ACTIONS_THREAT_INTELLIGENCE_HALF_LIFE_HOURS_VALUE="${ACTIONS_THREAT_INTELLIGENCE_HALF_LIFE_HOURS:-$(read_env_file_value ACTIONS_THREAT_INTELLIGENCE_HALF_LIFE_HOURS || true)}"
...
DATABASE_URL_FALLBACK="${DATABASE_URL_FALLBACK:-$(read_env_file_value DATABASE_URL_FALLBACK || true)}"
...
if [[ -n "$ACTIONS_THREAT_INTELLIGENCE_HALF_LIFE_HOURS_VALUE" ]]; then
  params+=("ActionsThreatIntelligenceHalfLifeHours=${ACTIONS_THREAT_INTELLIGENCE_HALF_LIFE_HOURS_VALUE}")
fi
