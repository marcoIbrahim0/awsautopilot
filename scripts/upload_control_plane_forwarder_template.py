#!/usr/bin/env python3
"""
Upload the control-plane forwarder CloudFormation template to S3 with versioned naming.

Usage (from project root):
  python scripts/upload_control_plane_forwarder_template.py [--version VERSION] [--bucket BUCKET] [--region REGION]
...
TEMPLATE_REL_PATH = "infrastructure/cloudformation/control-plane-forwarder-template.yaml"
S3_KEY_PREFIX = "cloudformation/control-plane-forwarder"


def main() -> None:
    parser = argparse.ArgumentParser(description="Upload control-plane forwarder template to S3 with version naming")
