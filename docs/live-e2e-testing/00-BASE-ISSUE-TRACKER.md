...
Last updated: 2026-03-01T21:59:29Z (Wave 7 Test 28 full remediation closure validation: blocked by root-credential gate, preservation pass)
...
| Wave 7 | 23–28 | 2026-03-01 | 3 | 0 | 1 | 2 |
| Wave 8 | 29–33 | — | — | — | — | — |
| Wave 9 | 34–35 | — | — | — | — | — |
| **TOTAL** | **35** | **2026-03-01** | **24** | **1** | **1** | **2** |
...
| 15 | Test 25 | 🟠 HIGH | IAM.4 root-key finding remained active after full closure flow | Target action/finding should resolve after successful root-key remediation apply and refresh | Wave 7 Test 25 created run `d7325d69-35ec-4d2b-8042-72d5b36f35ad` (`201`), refresh reached `completed` (`100%`), but apply failed under non-root credentials (`ERROR: root credentials are required to disable root access keys.`) and linked findings remained `NEW`. Wave 7 Test 28 revalidated same blocker (`run success`, `terraform apply=1`, final action `open`, finding `NEW`). | OPEN (Wave 7 Test 25 + Test 28 closure validation, 2026-03-01: `test-25-closure-20260301T154046Z`, `test-28-closure-20260301T215524Z`) |
...
| 28 | IAM inline + managed | N/A (B-series inline+managed preservation-focused scenario) | N/A (IAM.4 account-root target) | N/A (test scope is preservation under remediation attempt) | Yes (B3 trust policy, inline policy name/document, and managed-policy attachments unchanged pre/post apply) | BLOCKED (🟠 HIGH: run succeeded and bundle downloaded, but Terraform apply requires root credentials; target action/finding remained `open/NEW`) |
