...
- [Remediation-profile Wave 4 focused local validation on March 14, 2026 UTC](/Users/marcomaher/AWS%20Security%20Autopilot/docs/test-results/live-runs/20260314T193034Z-rem-profile-wave4-e2e/notes/final-summary.md) — stop for fixes on `master`: direct queue proof confirmed schema `v2` for single-run create/resend, shared grouped create, action-groups parity, and grouped resend; legacy schema `v1`, future-schema quarantine, direct-fix regression smoke, auth boundaries, and the `/actions/[id]` remediation UI/preview all passed on the isolated local master runtime, but `RPW4-06` found a blocking grouped duplicate-guard regression where differentiated override-map and `repo_target` requests returned `500` instead of creating distinct runs, and `RPW4-03` remained blocked because the local dataset exposes only one profile per strategy.
