# S3.2 migration variant (CloudFront + OAC + private S3) - Action: 9fa9f7b4-ac06-497e-bf34-ba31b8c98d51
...
locals {
  bucket_name                         = "security-autopilot-access-logs-696505809372-r221001"
  ...
  oac_name_seed                       = "${local.bucket_name}-9fa9f7b4ac06497ebf34ba31b8c98d51-20260402010529591546"
...
locals {
  ...
  existing_policy_document = try(
    jsondecode(data.aws_s3_bucket_policy.existing[0].policy),
    {
      Version   = "2012-10-17"
      Statement = []
    }
