...
Last updated: 2026-03-20T17:07:52Z (Wave 2 auth rerun PASS: remember-me persistence, refresh/session continuity, forgot-password, password change, temporary-login verification, and password revert all revalidated with fresh live evidence)
...
| Wave 2 | 02–04 | 2026-03-20 | 3 | 0 | 0 | 0 |
...
| 12 | Test 29 | Accounts page | `last_synced_at` | present and non-null (`2026-03-02T18:55:05.887601Z` pre, `2026-03-02T21:18:17.101603Z` post) | Freshness field is present and advanced after completed ingest refresh | ✅ FIXED (Wave 8 Test 29 rerun, 2026-03-02: `test-29-live-20260302T211811Z`) |
...
| 5 | Test 30 | 🟠 HIGH | Login endpoint not rate-limited after N attempts | 429 after threshold | Wave 8 Test 30 rerun observed deterministic transition for dedicated identity: wrong-password attempts `#1-#5` -> `401`, attempt `#6` -> `429`; correct password while blocked remained `429`; post-window correct login returned `200`. | ✅ FIXED (Wave 8 Test 30 rerun, 2026-03-02: `test-30-live-20260302T211956Z`) |
| 6 | Test 30 | 🟠 HIGH | No `Retry-After` header on 429 response | Header present | Wave 8 Test 30 rerun observed `retry-after` on throttled responses (`895`, `893`) and successful recovery after waiting the observed window (`200` login + `200` `/api/auth/me`). | ✅ FIXED (Wave 8 Test 30 rerun, 2026-03-02: `test-30-live-20260302T211956Z`) |
| 7 | Test 31 | 🟠 HIGH | Non-admin can invite users | 403 | Wave 8 Test 31 rerun observed member invite probe `403` (`Only admins can invite users`), no-auth probe `401`, wrong-tenant delete probe `404`, and admin control invite `201`. | ✅ FIXED (Wave 8 Test 31 rerun, 2026-03-02: `test-31-live-20260302T213603Z`) |
| 8 | Test 31 | 🟠 HIGH | Non-admin can delete accounts | 403 | Wave 8 Test 31 rerun observed member delete probe `403` (`Only admins can delete AWS accounts`), no-auth probe `401`, and wrong-tenant delete probe `404` after successful Tenant-B signup/auth. | ✅ FIXED (Wave 8 Test 31 rerun, 2026-03-02: `test-31-live-20260302T213603Z`) |
...
| 11 | Test 32 | 🟡 MEDIUM | Audit records contain secrets (token, role_arn, password) | No secrets in payload | Wave 8 Test 32 rerun full scan across `31` records found no matches for JWT/access-token/password/role_arn/AWS-key/private-key/webhook leakage patterns; contract rows kept `payload=null`. | ✅ FIXED (Wave 8 Test 32 rerun, 2026-03-02: `test-32-live-20260302T213947Z`) |
| 12 | Test 32 | 🟡 MEDIUM | Non-admin user can read audit log | 403 | Wave 8 Test 32 rerun observed member access `403` (`Only tenant admins can view audit logs.`) and no-auth probes `401` (`Not authenticated`) on `/api/audit-log` (base and filtered requests). | ✅ FIXED (Wave 8 Test 32 rerun, 2026-03-02: `test-32-live-20260302T213947Z`) |
...
| 16 | Test 26 | 🟠 HIGH | Complex S3 remediation overwrote existing non-risk bucket policy statements | Non-risk statements should remain preserved after remediation apply | Latest closure rerun reconfirmed non-risk statements were unchanged (`removed_non_risk=0`, `added_non_risk=0`) with only CloudFront remediation statement rotation (`removed_cloudfront=1`, `added_cloudfront=1`) | ✅ FIXED (Wave 7 Test 26 rerun, 2026-03-01: `test-26-closure-20260301T210222Z`) |
...
| 12 | Test 33 | PR proof C2 | No Terraform plan timestamp in README | Template doesn't include plan metadata | 🟠 HIGH | ✅ FIXED (Wave 8 Test 33 rerun, 2026-03-02: `test-33-live-20260302T214032Z`; README line includes `terraform_plan_timestamp_utc=2026-03-02T21:40:44+00:00` and consistency check confirms parseable + run-window alignment) |
| 13 | Test 33 | PR proof C5 | No preserved-config statement in B-series README | README template missing preservation section | 🟠 HIGH | ✅ FIXED (Wave 8 Test 33 rerun, 2026-03-02: `test-33-live-20260302T214032Z`; README includes `preserved_configuration_statement` and consistency check confirms metadata/apply-context coherence) |
...
| 22 | Test 25 | IAM root-key remediation apply path | Full closure cannot complete reliably: non-root apply fails immediately, and root delete-strategy apply self-breaks during execution | Generated IAM.4 bundle enforces root principal (`arn:aws:iam::*:root`) so IAM admin sessions cannot execute. In Test 28 continuation, root-disable apply succeeded but did not close action/finding, while root-delete apply failed with `InvalidClientTokenId` at generated `aws iam update-access-key` step, leaving root keys present and target unresolved after refresh. | 🟠 HIGH | OPEN (Wave 7 Test 25 + Test 28 continuation, 2026-03-02: `test-25-closure-20260301T154046Z`, `test-28-closure-rootapply-20260301T221832Z`, `test-28-closure-rootdelete-20260302T133128Z`) |
...
| 27 | Test 29 | Ingest sync trigger contract | Historical valid-call `500` regression resolved; endpoint now returns success contract (`200`) for valid in-tenant admin/member calls | Rerun reconfirmed trigger/progress/freshness behavior without regression and preserved deny-closed boundaries (`404/401/404`). | 🟠 HIGH | ✅ FIXED (Wave 8 Test 29 rerun, 2026-03-02: `test-29-live-20260302T211811Z`) |
...
| 26 | Complex S3 policy | Deterministic adversarial risk-state confirmed pre-run and target reopened in OPEN within SLA (`17s`) | N/A (test objective centers on preservation/closure after remediation apply) | N/A (test objective is policy-preservation during remediation) | Yes (delta-aware non-risk invariance pass and PAB hardened) | PASS (Track 1 PASS, Track 2 PASS) |
| 27 | Mixed SG rules | Yes (`B2` action/finding for `arch1_sg_app_b2` observed `open/NEW` pre-remediation) | N/A (single-target SG hardening scenario) | Yes (no benign-rule false positives after remediation; only public SSH removed) | Yes (`443` internal, `8080` admin host, and `5432` source-SG rules preserved; `22` `0.0.0.0/0` removed) | PASS (full closure validated: run success, Terraform `0/0/0/0`, refresh completed, action/finding resolved) |
| 28 | IAM inline + managed | N/A (B-series inline+managed preservation-focused scenario) | N/A (IAM.4 account-root target) | N/A (test scope is preservation under remediation attempt) | Yes (B3 inline + required managed policy still present after continuation checks; `required_safe_permissions_unchanged=true`) | BLOCKED (🟠 HIGH: non-root apply is root-gated, root-disable apply did not close, root-delete apply failed with `InvalidClientTokenId`; target action/finding remained `open/NEW`) |
...
| 11 | `/api/aws/accounts/{id}/ingest-sync` | Endpoint path is live; trigger returns `200` and `/ingest-progress` reached terminal `completed` (`progress=100`, `updated_findings_count=438`) | No missing contract observed in rerun evidence; `started_after` validation remains enforced (`422` when omitted) | Test 29 | ✅ FIXED (Wave 8 Test 29 rerun, 2026-03-02: `test-29-live-20260302T211811Z`) |
...
| 5 | Test 30 | Login throttle window duration now evidence-backed from live responses | None | ✅ FIXED (Wave 8 Test 30 rerun, 2026-03-02: first throttle response had `retry-after: 895`, blocked/spoofed probes stayed throttled, and post-window login recovered to `200`) |
...
| 2026-03-20 | Wave 2 auth session rerun | Revalidated live `remember_me` cookie persistence and fixed the live stale-`token_version` auth regression by refreshing `users.token_version` before issuing login/MFA session cookies; rerun evidence confirmed `POST /api/auth/login`, `POST /api/auth/refresh`, `GET /api/auth/me`, forgot-password, password change, temporary-password login, and final password revert all pass on `https://ocypheris.com` / `https://api.ocypheris.com`. | Wave 2 Tests 03-04 rerun | ✅ FIXED |
...
| 2026-03-01 | Section 3 #16 / Section 4 #25 / Section 4 #26 / Section 5 Test 26 / Section 6 #10 | Two-track Test 26 rerun after runtime deploy `20260301T201919Z` and visibility rollout flag enablement (`ACTIONS_EFFECTIVE_OPEN_VISIBILITY_ENABLED=true`) kept blocking path green (`run success`, Terraform `0/0/0/0`, final action/finding resolved, non-risk preservation unchanged). Track 2 remained open with explicit SLA metric: target did not appear in pre-run OPEN polling (`target_visible_in_open=false`, `elapsed_seconds=187`, `sla_seconds=180`) and required resolved fallback. | Wave 7 Test 26 rerun (`test-26-closure-20260301T202427Z`) | PARTIAL |
| 2026-03-01 | Section 4 #26 / Section 5 Test 26 / Section 6 #10 | Closure rerun after allowlist/forwarder fix for bucket-level PAB events (`PutBucketPublicAccessBlock`/`DeleteBucketPublicAccessBlock`) verified pre-run reopen visibility end-to-end: control-plane intake captured reset event, target finding transitioned to `effective_status=NEW` (`shadow=OPEN`), and target action reappeared in `status=open` within SLA (`elapsed_seconds=17`, `sla_seconds=180`) while Track 1 safety checks stayed green (`run success`, Terraform `0/0/0/0`, non-risk preservation pass). | Wave 7 Test 26 rerun (`test-26-closure-20260301T210222Z`) | ✅ FIXED |
...
| 2026-03-02 | Section 1 #14 / Section 2 #12 / Section 4 #27 / Section 6 #11 | Wave 8 Test 29 rerun revalidated ingest-sync and freshness contracts with new evidence prefix: valid in-tenant admin/member trigger calls returned `200`, progress reached terminal `completed` at poll `7` (`updated_findings_count=438`), `last_synced_at` advanced (`18:55:05.887601Z` -> `21:18:17.101603Z`), and boundary probes remained deny-closed (`invalid-account 404`, `no-auth 401`, `wrong-tenant 404`). | Wave 8 Test 29 rerun (`test-29-live-20260302T211811Z`) | ✅ FIXED |
| 2026-03-02 | Section 3 #5 / Section 3 #6 / Section 7 #5 / Section 8 T30-5 | Wave 8 Test 30 rerun revalidated login throttle contract with dedicated identity: wrong-password attempts `1-5` returned `401`, attempt `6` returned `429` (`retry-after: 895`), blocked-correct retry remained `429`, spoofed-XFF remained throttled, and post-window validation succeeded (`200` login + `200` `/api/auth/me`) after observed wait window (`895s`). | Wave 8 Test 30 rerun (`test-30-live-20260302T211956Z`) | ✅ FIXED |
| 2026-03-02 | Section 3 #7 / Section 3 #8 / Section 8 T31-7,T31-8 | Wave 8 Test 31 rerun revalidated non-admin authorization boundaries: member invite/delete probes both returned `403`, no-auth probes returned `401`, wrong-tenant delete probe returned `404`, admin invite control path returned `201`, and account post-state remained unchanged (no destructive success). | Wave 8 Test 31 rerun (`test-31-live-20260302T213603Z`) | ✅ FIXED |
| 2026-03-02 | Section 3 #11 / Section 3 #12 / Section 8 T32 | Wave 8 Test 32 rerun revalidated `/api/audit-log` contract with admin `200` (pagination + filters), deny-closed auth boundaries (`member=403`, `no-auth=401`), pagination overlap `0`, and no leakage hits across baseline/date/full scans (`31` records total, `payload` consistently `null`); focused Wave-7 keyword subset remained empty (`0` rows). | Wave 8 Test 32 rerun (`test-32-live-20260302T213947Z`) | ✅ FIXED |
| 2026-03-02 | Section 4 #12 / Section 4 #13 / Section 8 T33 | Wave 8 Test 33 rerun revalidated PR proof artifact completeness on fresh non-root run `4361b8d2-5600-48ad-b2b4-2cac683f721a`: README includes `terraform_plan_timestamp_utc=2026-03-02T21:40:44+00:00` (C2) and `preserved_configuration_statement` (C5), consistency checks passed (`c2_within_run_window_5min=true`, `strategy_matches_create_payload=true`, `apply_context_has_init_plan_apply_steps=true`), and ZIP auth boundaries remained closed (`200/401/404`). | Wave 8 Test 33 rerun (`test-33-live-20260302T214032Z`) | ✅ FIXED |
