...
2. The documented fallback browser login `maromaher54@gmail.com / Maher730` no longer signs in and instead returns `Verify your email before signing in`.
3. The documented tenant-admin login `marco.ibrahim@ocypheris.com / Maher730@` still fails on the live frontend and remains aligned with the prior task-history warning that the normal password path is broken.
