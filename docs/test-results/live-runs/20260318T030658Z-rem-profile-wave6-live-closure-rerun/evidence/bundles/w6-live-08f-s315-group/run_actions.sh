...
has_mirrored_aws_provider() {
  find -L "${TF_PROVIDER_MIRROR_DIR}/registry.terraform.io/hashicorp/aws/${AWS_PROVIDER_VERSION}" -type f -print -quit 2>/dev/null | grep -q .
}

bootstrap_provider_cache() {
  local bootstrap_dir marker
  marker="${CACHE_ROOT}/.aws-provider-${AWS_PROVIDER_VERSION}.ready"
  if [ -f "${marker}" ] && has_cached_aws_provider && has_mirrored_aws_provider && [ -s "${AWS_PROVIDER_LOCKFILE}" ]; then
    return 0
  fi
