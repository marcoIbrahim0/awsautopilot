...
- Internal profiles/branches added: `s3_bucket_block_public_access_manual_preservation`, `s3_bucket_block_public_access_review_public_policy_scrub`, `s3_migrate_cloudfront_oac_private_create_missing_bucket`, `s3_migrate_cloudfront_oac_private_manual_preservation`, and `s3_migrate_website_cloudfront_private_review_required`.
- Executable vs review/manual behavior: the standard BPA branch stays executable only when website/public-policy safety is proven; public non-website buckets now downgrade to Terraform-only `review_required_bundle` branch `s3_bucket_block_public_access_review_public_policy_scrub`; the CloudFront/OAC branch now stays executable when preservation is satisfied by captured existing bucket-policy JSON, by runtime-proven zero-policy state, by the Terraform-only apply-time merge path after a concrete capture failure, or by the dedicated create-missing-bucket branch when runtime proves the workload bucket no longer exists; the website branch stays executable only when runtime probes capture a simple S3 website configuration and the caller provides explicit Route53/ACM cutover inputs. Remaining standard/OAC preservation gaps still downgrade to `manual_guidance_only`, while website translation or DNS-input gaps downgrade to `review_required_bundle`.
- Tenant-default inputs used: none.
...
- Internal profiles/branches added: `s3_enforce_ssl_strict_deny_create_missing_bucket` and `s3_enforce_ssl_with_principal_exemptions_create_missing_bucket`.
- Executable vs review/manual behavior: executable output stays allowed when the current bucket policy is captured as merge-safe JSON, when runtime probes prove there is no current bucket policy, when runtime capture fails with a concrete bucket-policy access error and the resolver selects the Terraform apply-time merge path, or when runtime proves the workload bucket is missing and the resolver switches to the create-missing-bucket branch. Fully missing preservation evidence downgrades to `review_required_bundle`; explicit `preserve_existing_policy=false` downgrades to `manual_guidance_only`.
- Tenant-default inputs used: none.
- Preservation/runtime gates: `s3_policy_analysis_possible`, existing bucket-policy statement count, captured `existing_bucket_policy_json`, concrete `existing_bucket_policy_capture_error` for the apply-time Terraform path, parse errors, the runtime-only `GetBucketPolicyStatus -> NoSuchBucketPolicy` empty-policy proof, and shared target-bucket-missing preflight via `HeadBucket`. The adjacency gate now treats that concrete capture-error branch as sufficient preservation proof for Terraform only; direct CloudFormation generation still requires captured policy JSON, and the create-missing-bucket branch requires `bucket_creation_acknowledged=true` at create time.

### S3.11 `s3_bucket_lifecycle_configuration`

- Public compatibility strategy IDs preserved: `s3_enable_abort_incomplete_uploads`.
- Internal profiles/branches added: `s3_enable_abort_incomplete_uploads_create_missing_bucket`.
- Executable vs review/manual behavior: executable output is allowed when no lifecycle rules exist, when the captured lifecycle document already matches the abort-incomplete-uploads safe state, when captured existing lifecycle rules can be rendered as a safe additive Terraform merge, when runtime probes know the bucket but lifecycle capture fails concretely and the Terraform-only apply-time merge fallback is selected, or when runtime proves the workload bucket is missing and the resolver switches to the create-missing-bucket branch. Captured-but-unsupported lifecycle shapes downgrade to `review_required_bundle`; ambiguous lifecycle evidence with no concrete capture failure can still fall to `manual_guidance_only`.
- Tenant-default inputs used: none.
- Preservation/runtime gates: `existing_lifecycle_rule_count`, captured `existing_lifecycle_configuration_json`, additive-merge renderability for the requested `abort_days`, equivalent-abort-rule deduplication so the managed abort rule is not emitted twice, additive `preservation_summary.apply_time_merge/apply_time_merge_reason` metadata for the concrete capture-failure fallback, and shared target-bucket-missing preflight via `HeadBucket`. The create-missing-bucket branch requires `bucket_creation_acknowledged=true` at create time.

### S3.9 `s3_bucket_access_logging`

- Public compatibility strategy IDs preserved: `s3_enable_access_logging_guided`.
- Internal profiles/branches added: `s3_enable_access_logging_create_missing_bucket` and `s3_enable_access_logging_review_destination_safety`.
- Executable vs review/manual behavior: the compatibility branch stays executable only when bucket scope and destination safety are proven; when runtime proves the source bucket is missing, the resolver now switches to the create-missing-bucket branch; otherwise the resolver downgrades to `review_required_bundle` on the review branch.
...
- Preservation/runtime gates: source bucket scope must resolve from `action.target_id`, or fall back to `action.resource_id` when stale/account-scoped actions still carry the bucket ARN there; destination bucket must resolve and differ from the source bucket, and runtime checks must prove the destination bucket is reachable and safe. Shared target-bucket-missing preflight now applies to the source bucket itself; that create branch requires `bucket_creation_acknowledged=true` at create time. Account-scoped / ambiguous actions do not auto-generate a destination bucket. Terraform customer-run bundles can now adopt an already-owned destination bucket in place by switching to `adopt_existing_log_bucket=true` while still applying the support-bucket baseline.

### S3.15 `s3_bucket_encryption_kms`

- Public compatibility strategy IDs preserved: `s3_enable_sse_kms_guided`.
- Internal profiles/branches added: `s3_enable_sse_kms_guided_create_missing_bucket` and `s3_enable_sse_kms_customer_managed`.
- Executable vs review/manual behavior: the AWS-managed branch remains executable when target bucket scope is proven; when runtime proves the target bucket is missing, the resolver now switches the AWS-managed path to the create-missing-bucket branch; the customer-managed branch downgrades to `review_required_bundle` when KMS safety proof is incomplete and to `manual_guidance_only` when `kms_key_arn` is missing entirely.
- Tenant-default inputs used: `s3_encryption.mode` -> `kms_key_mode` and `s3_encryption.kms_key_arn` -> `kms_key_arn`.
- Preservation/runtime gates: target bucket scope proof from `action.target_id` with `action.resource_id` fallback for stale/account-scoped actions, shared target-bucket-missing preflight via `HeadBucket`, KMS ARN presence/shape, account-region validation, enabled-key validation, captured key-policy evidence, and captured grant evidence. The create-missing-bucket branch requires `bucket_creation_acknowledged=true` at create time.
...
## April 1, 2026 Follow-On Hardening

The March 18 closure package remains the authoritative Wave 6 gate outcome, but `master` gained additional follow-on hardening on April 1, 2026 for future grouped-bundle reruns and trust recovery:

- Mixed-tier grouped bundles no longer use a separate embedded runner. Both flat and mixed-tier grouped bundles now load the checked-in [`run_all.sh`](/Users/marcomaher/AWS%20Security%20Autopilot/infrastructure/templates/run_all.sh) template and vary only by `EXECUTION_ROOT`.
- The grouped bundle metadata contract now carries `runner_template_source = repo:infrastructure/templates/run_all.sh` plus `helper_bucket_inventory[]` so generated helper buckets remain visible to callback-time and post-apply verification.
- `S3.9` reruns now preserve the previously landed `adopt_existing_log_bucket` behavior on the canonical grouped runner path too, which closes the earlier mixed-tier gap where production bundles could still come from the older embedded path.
- Product-managed helper buckets now carry deterministic support-bucket tags, and inventory reconciliation suppresses `S3.9` only for tagged product-managed log sinks rather than exempting helper buckets broadly.
- Post-apply grouped callback handling now schedules a scoped reconcile refresh for both the original remediated resources and any tagged helper buckets recorded in grouped bundle metadata.
- `Tenant.external_id` is now the canonical immutable AssumeRole `ExternalId`, `aws_accounts.external_id` is treated as a mirrored compatibility field, and validation/service-readiness now return a specific trust-recovery payload when STS fails with `AccessDenied`.

Deployment note:

- These April 1, 2026 changes are checked in on `master`, but they still require backend/runtime deployment plus a fresh live canary before they can be treated as current production behavior.
