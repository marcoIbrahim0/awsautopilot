...
- `POST /api/actions/{id}/trigger-reeval` is the targeted post-apply refresh contract for Action Detail `Refresh State`; browser callers should use it first and fall back to scoped `POST /api/aws/accounts/{account_id}/ingest` only when the API returns `400 Immediate re-evaluation not supported`.
...
- `GET /api/remediation-runs` accepts additive `include_group_related=true` when `action_id` is present; that union view returns both direct runs for the action and grouped bundle runs linked through the persistent action-group membership so Action Detail history stays truthful for non-representative member actions.
...
- `POST /api/remediation-runs/group-pr-bundle` now writes the internal `action_group_runs` row and grouped reporting metadata at create time, matching `POST /api/action-groups/{group_id}/bundle-run`, so grouped history and artifact links exist immediately instead of only after later backfill/worker reconciliation.
