> Scope date: 2026-02-25 historical validation snapshot.
>
> Current contract note (2026-03-19): current `master` is PR-only. Historical direct-fix references below are preserved to explain the original production-readiness extraction inputs, not the live product contract.

SECTION 1 — CONTROL INVENTORY SUMMARY
| Control ID | Control Name | AWS Service | Action Type (historical direct-fix / pr-bundle snapshot) |
|------------|-------------|-------------|--------------------------------------|
| S3.1 | S3 account public access hardening | s3control | historical direct-fix + pr-bundle |
| SecurityHub.1 | Enable Security Hub | securityhub | historical direct-fix + pr-bundle |
| GuardDuty.1 | Enable GuardDuty | guardduty | historical direct-fix + pr-bundle |
...
| EC2.7 | Enable EBS default encryption | ec2 | historical direct-fix + pr-bundle |
...
| direct_fix | historical `direct_fix` mode marker (preview path) | direct-fix | UNKNOWN |
| pr_bundle | PR bundle mode | pr-bundle | UNKNOWN |
| s3_block_public_access | S3 account public access hardening | historical direct-fix + pr-bundle | API: s3control.put_public_access_block; IaC: aws_s3_account_public_access_block, AWS::CloudFormation::WaitConditionHandle |
| enable_security_hub | Enable Security Hub | historical direct-fix + pr-bundle | API: securityhub.enable_security_hub; IaC: aws_securityhub_account, AWS::SecurityHub::Hub |
| enable_guardduty | Enable GuardDuty | historical direct-fix + pr-bundle | API: guardduty.create_detector, guardduty.update_detector; IaC: aws_guardduty_detector, AWS::GuardDuty::Detector |
...
| ebs_default_encryption | Enable EBS default encryption | historical direct-fix + pr-bundle | API: ec2.enable_ebs_encryption_by_default, ec2.modify_ebs_default_kms_key_id; IaC: aws_ebs_encryption_by_default, aws_ebs_default_kms_key, Custom::EbsDefaultEncryption |
...
- Controls showing historical direct-fix coverage in the snapshot: 4
...
- Action ID `direct_fix`: historical mode-marker AWS API or IaC change is UNKNOWN.
