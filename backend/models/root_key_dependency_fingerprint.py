...
class RootKeyDependencyFingerprint(Base):
    """Dependency snapshots used to decide auto/manual root-key remediation paths."""

    __tablename__ = "root_key_dependency_fingerprints"
    ...
    __table_args__ = (
        ...
        CheckConstraint("retry_count >= 0", name="ck_root_key_dependency_retry_non_negative"),
        Index("ix_root_key_dependency_tenant_run", "tenant_id", "run_id"),
        Index("ix_root_key_dependency_tenant_unknown", "tenant_id", "unknown_dependency", "status"),
    )
    ...
    status: Mapped[RootKeyDependencyStatus] = mapped_column(
        SAEnum(RootKeyDependencyStatus, name="root_key_dependency_status", create_type=False),
        nullable=False,
    )
    ...
    fingerprint_payload: Mapped[dict | list | None] = mapped_column(JSONB, nullable=True)
    unknown_dependency: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    run: Mapped["RootKeyRemediationRun"] = relationship(
        ...
        foreign_keys=[run_id],
        lazy="selectin",
        back_populates="dependency_fingerprints",
    )
