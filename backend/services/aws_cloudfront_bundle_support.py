"""Support helpers for S3.2 CloudFront/OAC executable PR bundles."""
from __future__ import annotations

from textwrap import dedent

AWS_CLOUDFRONT_OAC_DISCOVERY_SCRIPT_PATH = "scripts/cloudfront_oac_discovery.py"
AWS_CLOUDFRONT_OAC_DISCOVERY_QUERY_PATH = "cloudfront_reuse_query.json"


def aws_cloudfront_oac_discovery_script_content() -> str:
    """Return the S3.2 CloudFront/OAC adopt-or-create discovery helper."""
    return dedent(
        """\
        #!/usr/bin/env python3
        \"\"\"
        ...
        def validate_distribution(
            ...
            *,
            ...
            return {
                ...
                "reuse_distribution_id": str(distribution.get("Id") or "").strip(),
                "reuse_distribution_arn": str(distribution.get("ARN") or "").strip(),
                "reuse_distribution_domain_name": str(distribution.get("DomainName") or "").strip(),
                "reuse_oac_id": oac_id,
            }
        ...
        def validate_named_oac(
            ...
            if not matches:
                return {"cloudfront_reuse_mode": "create"}
            if len(matches) != 1:
