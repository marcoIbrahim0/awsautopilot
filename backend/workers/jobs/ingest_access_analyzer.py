"""
IAM Access Analyzer ingestion job handler (Step 2B.1).

Fetches Access Analyzer findings via assumed role, normalizes to our finding shape,
and upserts into Postgres with source='access_analyzer'. Optionally enqueues
compute_actions for the same tenant/account/region.
"""
...
from backend.workers.services.access_analyzer import (
...
def _upsert_one(
    ...
    """Upsert one Access Analyzer finding by (finding_id, account_id, region, source)."""
    fid = data["finding_id"]
...
def execute_ingest_access_analyzer_job(job: dict) -> None:
    """
    Process an ingest_access_analyzer job: assume_role → fetch Access Analyzer → upsert.
    ...
    with session_scope() as session:
        ...
        logger.info(
            "Upserting %d Access Analyzer findings tenant_id=%s account_id=%s region=%s",
            n_total,
            tenant_id,
            account_id,
            region,
        ...
        for i, raw in enumerate(findings_raw):
            try:
                data = normalize_aa_finding(raw, account_id, region, tenant_id)
                ...
                error_count += 1
                logger.warning(
                    "Duplicate or constraint error for finding id=%s: %s",
                    raw.get("id"),
                    e,
                )
                ...
                error_count += 1
                logger.exception(
                    "Failed to upsert Access Analyzer finding id=%s: %s",
                    raw.get("id"),
                    e,
                )
            if (i + 1) % 50 == 0:
                logger.info(
                    "Access Analyzer upsert progress: %d/%d (new=%d updated=%d errors=%d)",
                    i + 1,
                    n_total,
                    new_count,
                    updated_count,
    ...
    logger.info(
        "ingest_access_analyzer complete tenant_id=%s account_id=%s region=%s "
    ...
    if settings.has_ingest_queue:
        try:
            ...
            logger.info(
                "Enqueued compute_actions after Access Analyzer ingest tenant_id=%s account_id=%s region=%s",
                tenant_id,
                account_id,
                region,
            )
        except ClientError as e:
            logger.warning("Failed to enqueue compute_actions after Access Analyzer ingest: %s", e)
        except Exception as e:
            logger.warning("Failed to enqueue compute_actions after Access Analyzer ingest: %s", e)
